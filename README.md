...
- **Application PDF auto-fill/OCR is tuned to one real layout** (a printed/exported screenshot with "Label" captions above boxed values — see `src/lib/pdf-parse.ts`), since that's the only real sample available. It degrades gracefully (skips a field/the headshot rather than guessing) when it doesn't recognize the layout, but hasn't been tested against genuinely different application formats.
- Its two dependencies (`mupdf`, `tesseract.js`) load large `.wasm` binaries at runtime via a plain file read rather than `require()`/`import`, which Vercel's automatic function file-tracing doesn't follow — the binaries silently didn't make it into the deployed function, crashing the request. `vercel.json`'s `functions["api/handler.js"].includeFiles` explicitly includes both packages' full directories to fix this. If either package is ever upgraded and this resurfaces (or a new WASM-based dependency is added), this is the mechanism to reach for.
...
- **First login / admin reset**: officers don't self-register. A district-tier officer (District Director / Chief Dean of Membership Intake / Chief Administrator) opens **Officer Access** (`/admin`, linked from the user menu) and clicks "Reset Password" / "Issue Password" for any officer — this generates a random 12-character temp password shown once on screen and, if Resend is configured (`RESEND_API_KEY`/`RESEND_FROM_EMAIL` — see "Deployment"), emails it to the officer directly. Without Resend configured (or if the send fails), it falls back to the admin relaying it out of band (phone/text/in person) same as before. The officer is forced into a "Set a New Password" modal on next sign-in and can't dismiss it or use the app until they set their own password (min 10 characters).
...
- **Environment variables** — set these on the Vercel project (Project Settings → Environment Variables), for both Production and Preview:
  ...
  - `RESEND_API_KEY`, `RESEND_FROM_EMAIL` — optional. Enables emailing officer temp passwords and candidate status-change notifications via [Resend](https://resend.com); `RESEND_FROM_EMAIL` must be on a domain verified in that Resend account. Unset means those actions still work, just without the email — see `.env.example`.
...
- **Roster updates after officers already have credentials**: officers are managed from the district-tier **Officer Access** admin screen (add/remove/edit — backed by the `officers` table, see `migrations/0003_officers.sql`), not by editing code. If you add an officer there after `/api/auth/bootstrap` has already run once, bootstrap will only seed the *new* officer it doesn't recognize yet — it won't touch existing rows. A removed officer is deactivated (not deleted) so their old credential/audit rows stay intact and reachable if reactivated; use **Officer Access** to issue a fresh password for any officer whose email changed.
...
- Officer sign-in is now real password auth (hashed+salted, lockout, forced first-change — see "Auth & access control"), but there's no SSO/email-authentication provider integration. Temp-password and candidate-status-change emails go out via Resend if configured (see "Deployment"); without it, temp passwords fall back to being relayed by a district-tier officer out of band. The "Sign in with Alpha Member Portal" button on `/signin` is still a disabled placeholder.
