...
export function makeCandidate(input: ManualCandidateInput): Candidate {
  ...
  return {
    ...
    checks: {
      ...
      gpaMin: chapter?.type === 'alumni'
        ? { pass: true, value: 'Not applicable — alumni candidates are not subject to the GPA requirement' }
        : { pass: gpa > 2.5, value: gpa > 2.5 ? `${gpa.toFixed(2)} > 2.50` : `${gpa.toFixed(2)} does not exceed the 2.50 minimum` },
